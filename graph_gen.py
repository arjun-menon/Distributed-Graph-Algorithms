...
def random_graph(max_nodes, edge_count):
    all_edges = enumerate_edges(max_nodes)

    return random.sample(all_edges, edge_count)

def random_weighted_graph(max_nodes, edge_count):
    def edge(n1, n2, w):
            return (n1, n2, {'weight':w})

    rg = random_graph(max_nodes, edge_count)
...
def weighted_graph_to_file(g, fname):
...
if __name__ == "__main__":
    ...
    n = args.nodes[0]
    print("Maximum number of edges is %d." % max_edges(n))
    e = args.edges[0]
    print("Out of which %d edges will be randomly chosen." % e)
    f = args.file_name

    rg = random_weighted_graph(n, e)

    weighted_graph_to_file(rg, f)
