...
I largely followed the pseudocode as a guide, rather than following it directly. For my implementation I tried, to the maximal extent possible, to work out the lower level details myself, while simply following the high-level details of the algirithm as described and shown in the diagram above.. The benefit of following the high-level explanation was that, I was able to keep the *big picture* in my head (all at once.) I found that impossible to do with the pseudocode (or the low-level description, which contains lots of minute details intricately connected to each other.) I found I was able to understand part of it at a time, but to hold the whole thing in my head at once was impossible.
...
The great thing about this final and ultimate test case is that is _tests merges between ***Level-2*** fragments_. The final fragment formed at the end of this test case is a **Level-3 fragment**. The `graph-3-ouptput.txt`  file in this directory shows the output generated by `MST.dis` while solving this test case.
